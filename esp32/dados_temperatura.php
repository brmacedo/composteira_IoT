...
if($tipo_grafico == 1){
    ...
    $temperatura   = mysqli_query($conn, 'SELECT temperatura FROM ( SELECT * FROM composteira ORDER BY id DESC LIMIT '.$qtd_exb_valores.') Var1 ORDER BY ID ASC');
}
else{
    ...
    $temperatura   = mysqli_query($conn, 'SELECT temperatura FROM (SELECT * FROM composteira WHERE time BETWEEN "'.$inicio_grafico.'" AND "'.$fim_grafico.'" ORDER BY id DESC) Var1 ORDER BY ID ASC');
}
...
<script>
        ...
        var tempDados = {
            ...
            datasets: [
            {
                ...
                fill: true,
                lineTension: 0.1,
                backgroundColor: "rgba(150, 50, 0, .2)",
                borderColor: "rgba(150, 10, 0, .7)",
                borderCapStyle: 'round',
                borderDash: [],
                borderDashOffset: 0.0,
                borderJoinStyle: 'round',
                pointBorderColor: "rgba(150, 10, 130, .7)",
                pointBackgroundColor: "#fff",
                pointBorderWidth: 1,
                pointHoverRadius: 5,
                pointHoverBackgroundColor: "rgba(150, 10, 130, .7)",
                pointHoverBorderColor: "rgba(150, 10, 130, .7)",
